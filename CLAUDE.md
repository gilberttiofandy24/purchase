...
- `WeeklyNetSalesRate` — a `rate` multiplier per `(store_id, week_start_date)`. Net Sales is **derived**, not manually entered: `net_sales = gross_sales_total × rate`. This single derived Net Sales feeds *both* the Purchase Ratio and Labour Cost % calcs (there is no separate manual net-sales figure anymore).
...
- `WeeklyLabourRate` — manually-entered `weekday_rate`/`weekend_rate` per `(store_id, week_start_date)`. This is the **gross rate** (already includes superannuation) used directly in the labour cost formula — not a base wage that gets multiplied further.

Both `WeeklyNetSalesRate` and `WeeklyLabourRate` follow the same **rate carry-forward** pattern: resolved by querying `store_id` + the closest `week_start_date <= this week` (so a store doesn't need to re-enter its rate every week — it sticks until changed); if a store has never set one, it falls back to a `fallback*` constant in `purchase.go` (`fallbackNetSalesRate = 0.9`, `fallbackWeekdayRate`/`fallbackWeekendRate`). The resolved values are echoed back in the weekly-report response (`net_sales_rate`, `weekday_rate`, `weekend_rate`) so the frontend can reuse them for optimistic recompute without hardcoding.

The core endpoint is `GET /api/purchase/weekly-report?store_id=&week_start_date=` (in `internal/handlers/purchase.go`) — `week_start_date` must be a Monday (validated). It fans out to query suppliers/purchases, gross sales, employees/labour-hours, and the applicable net-sales and labour rates for that Mon–Sun range, zero-fills missing days, and computes in one pass:
...
- `net_sales` = `gross_sales_total × net_sales_rate` (see `WeeklyNetSalesRate` above)
- `purchase_ratio_pct` = grand total purchase ÷ `net_sales`
- `labour_cost_pct` = weekly labour total ÷ `net_sales`
- per-day labour cost = `(sum of that day's hours across all employees) × rate`, where `rate` is the resolved `weekday_rate` or `weekend_rate` depending on `time.Weekday()`. `staff_count` for a day in `labour_daily` is *derived* (count of employees with hours > 0 that day), not a stored field.

All other writes (`PUT /purchase/entry`, `/gross-sales`, `/net-sales/rate`, `/labour/hour-entry`, `/labour/rate`) are idempotent upserts keyed by the natural unique index (see `clause.OnConflict` calls), matching the frontend's per-cell autosave — the frontend never tracks row IDs for these, only the natural key.
...
The purchase-report, labour-cost, and summary pages (`app/(dashboard)/{purchase-report,labour-cost,summary}/page.tsx`) all drive everything off two URL search params, `store_id` and `week` (a Monday date, `yyyy-LL-dd`) — changing store or week pushes a new URL, which changes the TanStack Query key and refetches. All three call the same `GET /purchase/weekly-report` endpoint; the summary page (`/summary`) is a mostly-read-only rollup of that same report shape (one row per metric — Gross Sales/Net Sales/Purchase/Labour — across the week, plus weekly totals and both ratios); it owns the `GrossSalesRow` grid (moved here from purchase-report, since Gross Sales feeds the derived Net Sales) and `NetSalesRateInput`, but Purchase and Labour stay editable only on their own pages (`purchase-report`, `labour-cost`). Labour cost was split into its own route/page (rather than staying a row on purchase-report) because it's gated behind `LabourOtpGate` — a client-side dialog that POSTs a 6-digit code to `/labour/verify-otp` (checked against the `LABOUR_OTP_CODE` env var) and, once correct, sets a `sessionStorage` flag so the gate doesn't re-prompt for the rest of the browser session. `LabourGrid.tsx` mirrors `PurchaseGrid.tsx` exactly (per-employee rows instead of per-supplier), including the inline "add employee" row (calls `createEmployee`, same as the grid's inline "add supplier" calls `createSupplier`). `AmountInput` and `HourInput` are uncontrolled-ish input cells keyed by their current value (`key={value}`) so they remount and show the latest server value after a refetch, but save `onBlur` via a mutation that does an **optimistic `setQueryData` recompute** of the whole report shape (totals/percentages/ratios) rather than waiting for a refetch — see the `onMutate` handlers in `PurchaseGrid.tsx`, `GrossSalesRow.tsx`, `LabourGrid.tsx`, `LabourRateInput.tsx`, `NetSalesRateInput.tsx`. If you add a new editable field to the report, follow this same optimistic-recompute pattern rather than just invalidating (invalidating alone causes a visible flash/lag on every keystroke-blur).
